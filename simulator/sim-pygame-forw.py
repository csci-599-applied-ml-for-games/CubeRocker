...
class Field(object):
    ...
    def check_crash(self, game, player_x, player_y):
        return self.grid[player_y][player_x] == 1 # grid is height first
        ...
        while not game.crash:
            ...
            if game_epoch >= 19:
                ...
                if game.crash:
                    ...
                    if_remember = True
                    # print("remember this move with reward {}".format(reward))
                print("actual move {} to ({}, {}) gets reward {} - remember {}".format(final_move, player1.x, player1.y, reward, if_remember))

                # explore other move
                if final_move == 0: # no
                    # 1 left
                    explore_moves(game, field0, agent, player1, state_old, 1, max(0, player1.x - 1), player1.y)
                    # 2 right
                    explore_moves(game, field0, agent, player1, state_old, 2, min(player1.x + 1, 21), player1.y)
                elif final_move == 1: # left
                    # 0 no
                    explore_moves(game, field0, agent, player1, state_old, 0, min(player1.x + 1, 21), player1.y)
                    # 2 right
                    explore_moves(game, field0, agent, player1, state_old, 2, min(player1.x + 2, 21), player1.y)
                elif final_move == 2: # right
                    # 0 no
                    explore_moves(game, field0, agent, player1, state_old, 0, max(0, player1.x - 1), player1.y)
                    # 2 right
                    explore_moves(game, field0, agent, player1, state_old, 1, max(0, player1.x - 2), player1.y)
...
def explore_moves(game, field0, agent, player1, state_old, m, x, y):
    if_crash = field0.check_crash(game, x, y)
    reward = agent.set_reward(player1, if_crash, m)
    state_new = agent.get_state_pos(game, x, y, field0)
    agent.train_short_memory(state_old, m, reward, state_new, if_crash)
    if_remember = False
    if if_crash:
        agent.remember(state_old, m, reward, state_new, if_crash)
        if_remember = True
    elif m == 0 and randint(1, 20) < 20:
        agent.remember(state_old, m, reward, state_new, if_crash)
        if_remember = True
    elif m != 0 and randint(1, 20) < 20:
        agent.remember(state_old, m, reward, state_new, if_crash)
        if_remember = True
    print("explore move {} to ({}, {}) gets reward {} - remember {}".format(m, x, y, reward, if_remember))
